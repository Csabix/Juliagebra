...
function Triangle(a,b,c)
    return ParametricSurface(range(0,1,3),range(0,1,3),[a,b,c]) do u,v,a,b,c
...
SPHERE_HIDDEN_DETAIL = 15
SPHERE_TRIANGLE_COUNT = (SPHERE_HIDDEN_DETAIL-1)*(SPHERE_HIDDEN_DETAIL-1) + (SPHERE_HIDDEN_DETAIL-1)*(SPHERE_HIDDEN_DETAIL-1)
SPHERE_HITBOX_LOAD = false
...
surface = ParametricSurface(range(-10,10,50), range(-10,10,50)) do u,v
    x = u
    y = v
    z = (u^2 + v^2) * -0.05
...
for i in 1:392
    ParametricCurve(range(0,1,2), [sphere2SurfaceIntersection]; width=3.0) do t, sphere2SurfaceIntersection
