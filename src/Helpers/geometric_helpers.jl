...
function angle_bisector(a::Vec3D,b::Vec3D,c::Vec3D)::PLine
    ab = a-b
    cb = c-b
    v = b+(ab/norm(ab,ab) + cb/norm(cb,cb))
    return PLine(b,v)
end
...
"""
angle_bisector_plane_internal(coord::Vec3D,plane1::PrimitiveWithNormal,plane2::PrimitiveWithNormal)::Tuple{Vec3D,Vec3D} =
