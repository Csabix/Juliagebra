...
function update_color!(self::TriangleRenderer, ref::UInt32, color::Vec3F, transparent::Bool)
    alpha = transparent ? 0.5f0 : 1.0f0
    id_val = self.color_ids[ref][2]
    self.color_ids[ref] = Vec2T{UInt32}(packUnorm4x8(Vec4F(color[1],color[2],color[3],alpha)), id_val)
...
function opaque(self::TriangleRenderer,cam::Camera,shrd::SharedData)::Nothing
    ...
    for i in 1:length(self.buffers)
        ...
        uniform(self.shader_opaque,"MVP",vp*self.matrices[i])
