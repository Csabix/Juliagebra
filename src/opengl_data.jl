...
mutable struct OpenGLData <: ObserverBuilderDNA
    ...
    function OpenGLData(glfw::GLFWData,shrd::SharedData)
        ...
        combinerShader  = ShaderProgram(sp("dflt_combiner.vert"),sp("dflt_combiner.frag"),["At","WH","frameTex","depthTex","Eye","NEAR_FAR_DISTANCE_DISTANCE_LOG"])
...
function update!(self::OpenGLData,cam::Camera)
    ...
    setUniform!(self._combinerShader,"NEAR_FAR_DISTANCE_DISTANCE_LOG",Vec4F(cam._zNear,cam._zFar,norm(cam._at - cam._eye),1.0/distance))
    setUniform!(self._combinerShader,"WH",Vec2F(self._shrd._width,self._shrd._height))
    #setUniform!(self._combinerShader,"W_H_DISTANCE_DISTANCE_LOG",Vec4F(cam.width,cam.height,norm(cam._at - cam._eye),1.0/distance))
